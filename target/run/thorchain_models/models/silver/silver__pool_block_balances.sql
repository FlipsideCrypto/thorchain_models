      ...
      create or replace transient table THORCHAIN_DEV.silver.pool_block_balances copy grants as
      (select * from(
...
FROM
  ...
  AND bpd.pool_name = p.pool_name
    ...
    alter table THORCHAIN_DEV.silver.pool_block_balances cluster by (_inserted_timestamp::DATE);